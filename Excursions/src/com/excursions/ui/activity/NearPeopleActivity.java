...
import com.excursions.adapter.NearPeopleAdapter;
import com.excursions.bean.User;
import com.excursions.ui.customview.XListView;
import com.excursions.ui.customview.XListView.IXListViewListener;
...
public class NearPeopleActivity extends ActivityBase implements
	...
	private void initNearByList(final boolean isUpdate) {
		...
		if (!myApplication.getLatitude().equals("")
				&& !myApplication.getLongtitude().equals("")) {
			double latitude = Double.parseDouble(myApplication.getLatitude());
			double longtitude = Double.parseDouble(myApplication
					.getLongtitude());
	...
	private void queryMoreNearList(int page) {
		double latitude = Double.parseDouble(myApplication.getLatitude());
		double longtitude = Double.parseDouble(myApplication.getLongtitude());
		// 查询10公里范围内的性别为女的用户列表
